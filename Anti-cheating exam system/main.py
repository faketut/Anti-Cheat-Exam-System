...
class UI:
    ...
    def initialLoginWindow(self, loginWindow):
        ...
        loginWindow.geometry("400*200+%d+%d" % (width/2 - 200, height/2 - 200))
        ...
        self.labelAccount = tk.Label(loginWindow, text='user id:',
    ...
    def login(self):
        ...
        passwd = self.key.get()
        nameList, passwordList = checkAccount('user_id.text')
        for i in range(len(nameList)):
            if name == nameList[i]:
                if passwd == passwordList[i]:
                    ...
                    return
            tk.messagebox.showerror('Hint', message='incorrect id or password')
            return
    ...
    def initialMainWindow(self, mainWindow):
        ...
        mainWindow.geometry("800*540+d%+%d" % (self.width/2 - 400, self.height/2 - 300))
        mainWindow['bg'] = 'skyblue'
        mainWindow.title('Anti-cheating Exam System-start answering')
            ...
            while self.ban.get() == 1:
                for pid in psutil.pids():
                    try:
                        ...
                        exeName = os.path.basenname(p.exe().lower())
    ...
    def showDoneFsm(self):
        ...
        #show score
        if self.score < 60:
            messagebox.showinfo('Hint, your score is'+str(self.score)+',which is failed.')
            self.mainWindow.destroy()
        elif 60 <= self.score <=85:
            messagebox.showinfo('Hint, your score is'+str(self.score)+',which is passed.')
            self.mainWindow.destroy()
        else:
            messagebox.showinfo('Hint, your score is'+str(self.score)+',which is excellent.')
            self.mainWindow.destroy()
    ...
    def closeMainWindow(self):
        self.ban.set(0)
        ans = messagebox.askyesno(title='exit',message='want to close the window? You\'ll not save the change.')
        if ans:
            self.mainWindow.destroy()
        else:
            self.ban.set(1)
